...
if __name__ == "__main__":
    ...
    realdata = ICDAR2015(args.icdar2015_dir, target_size=768, viz=False)
    ...
    while train_step < whole_training_step:
        ...
        for batch_index, (real_images, real_region_label, real_affi_label, real_confidence_mask, real_confidences) \
            ...
            images = Variable(real_images).cuda()
            real_region_label = real_region_label.type(torch.FloatTensor)
            real_affi_label = real_affi_label.type(torch.FloatTensor)
            region_image_label = Variable(real_region_label).cuda()
            affinity_image_label = Variable(real_affi_label).cuda()
            confidence_mask_label = Variable(real_confidence_mask).cuda()
